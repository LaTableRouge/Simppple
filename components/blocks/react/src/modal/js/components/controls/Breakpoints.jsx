...
export default function Breakpoints(props) {
	...
	const addBreakpoint = (e) => {
		...
		if (!existingBreakpoint) {
            ...
            setMessage(`${__('Le point de rupture', 'highfive')} "${formData.breakpoint}" ${__('existe déjà.', 'highfive')}.`)
		}
	}
	...
	const updateBreakpoint = (breakpoint, attributeToUpdate, newValue) => {
		...
		if (existingBreakpointIndex !== undefined) {
            ...
            setMessage(`${__('Le point de rupture', 'highfive')} "${breakpoint}" ${__('n\'a pas été trouvé.', 'highfive')}`)
		}
	}
	...
	return (
		<>
			<ToggleControl
                label={__('Activer les points de rupture responsive', 'highfive')}
			...
			{hasBreakpointsEnabled && (
				<BaseControl className="breakpoints__control">
					<form onSubmit={addBreakpoint} className="control__form">
						<UnitControl units={[{ value: 'px', label: 'px', default: 0 }]} label={__('Ajouter un point de rupture', 'highfive')} placeholder={__('768', 'highfive')} required={true} name="breakpoint" />
						<Button type="submit" className="button button-primary" aria-label={__('Ajouter un point de rupture', 'highfive')}>
							+
						</Button>
					</form>
					...
					{attributes.breakpoints.length > 0
						&& attributes.breakpoints.map((object, key) => (
							<div id={`bp-${object.breakpoint}`} key={key} className="control__item">
								<PanelBody title={`${__('Point de rupture', 'highfive')}: ${object.breakpoint}`} initialOpen={false}>
									{/* TODO : voir comment réutiliser les composant du panel de contrôle pour éviter la duplication de code */}

									<PanelBody initialOpen={false} title={__('Configuration globale', 'highfive')}>
										<UnitControl
											value={ object.config['max-width'] || '' }
											help={ __('Largeur maximum', 'highfive') }
											units={
												[
									...
									<PanelBody initialOpen={false} title={__('Configuration du bouton de fermeture', 'highfive')}>
										<UnitControl
											value={ object.config['close-button-size'] || '' }
											help={ __('Taille du bouton', 'highfive') }
											units={
												[
													{ value: 'px', label: 'px', default: 0 }
												]
										...
										<UnitControl
											...
											help={ __('L\'espace entre le texte du bouton et sa bordure', 'highfive') }
											units={
												[
													{ value: 'px', label: 'px', default: 0 }
												]
									...
									<Button
                                        ...
                                        aria-label={`${__('Supprimer le point de rupture', 'highfive')}: ${object.breakpoint}`}
										onClick={() => {
											removeBreakpoint(object.breakpoint)
										}}
									>
                                        {__('Supprimer la configuration du point de rupture.', 'highfive')}
									</Button>
								</PanelBody>
							</div>
						))}
