...
window.addEventListener('DOMContentLoaded', () => {
  const containers = document.querySelectorAll('.wp-block-highfive-modal:not(.is-init)')
  if (containers.length) {
    containers.forEach((container) => {
      ...
      if (closeElements.length) {
        closeElements.forEach((element) => {
          element.addEventListener('click', (e) => {
            ...
            document.documentElement.classList.remove('highfive-modal-deployed')
          })
        })
      }
      ...
      if (openAuto === 'true') {
        ...
        document.documentElement.classList.add('highfive-modal-deployed')
